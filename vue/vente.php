        ...
        <?php
           ...
           if(!empty($articles)&& is_array($articles)){
            foreach($articles as $key=>$value){
                ?>
              <option value="<?=$value['id']?>" data-prix="<?=$value['prix_unitaire']?>"> <?=$value['nom_article']." - ".$value['quantite']." disponible"?></option>
            <?php
              }
           }
        ...
        <?php
           ...
           if(!empty($clients)&& is_array($clients)){
            foreach($clients as $key=>$value){
                ?>
           <option value="<?=$value['idclient']?>"> <?=$value['nom']." ".$value['prenom']?></option>
           <?php
              }
           }
           ...
           if(!empty($ventes)&& is_array($ventes)){
            foreach ($ventes as $key => $value) {
               ...
               <tr>
                  ...
                  <td><a href="recuVente.php?id=<?=$value['id']?>"><i class='bxr  bx-receipt'></i> </a>
...
function annuleVente(idVente,idArticle,quantite){
   if(confirm("Veulez-vous vraiment annuler cette vente? ")){
